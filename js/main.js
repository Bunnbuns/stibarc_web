function toLink(id,item) {
	try {
        ...
        document.getElementById("list").innerHTML += '<div class="post" onclick="goToPost('+id+');"><a style="font-size:100%;text-decoration:none;" href="post.html?id='+id+'"><b>'+title+'</b></a><div class="meta" style="margin-top:3px;"><span>Posted by: <a href="user.html?id='+item['poster']+'">'+item['poster']+'</a><br>&#8679; '+item['upvotes']+' &#8681; '+item['downvotes']+'</a></span></div></div>';
		lastid = id;
	} catch (err) {
		console.log(err);
	}
...
function toFollowLink(id,item) {
	try {
		if (item['deleted']) {item['title'] = "Post deleted"}
		var title = item['title'].replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
		document.getElementById("followlist").innerHTML += '<div class="post" onclick="goToPost('+id+');"><a style="font-size:100%;text-decoration:none;" href="post.html?id='+id+'"><b>'+title+'</b></a><div class="meta" style="margin-top:3px;"><span>Posted by: <a href="user.html?id='+item['poster']+'">'+item['poster']+'</a><br>&#8679; '+item['upvotes']+' &#8681; '+item['downvotes']+'</a></span></div></div>';
		lastfollowid = id;
	} catch (err) {
		console.log(err);
	}
